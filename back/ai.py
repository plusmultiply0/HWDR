...
def to_predict(input):
    # 加载模型
    svm_model_all_train_load = joblib.load("svm_model_all_training.pkl")
    #预处理
    # 黑底白字+居中
    deal_input = center_image(input).reshape(1,784)
    # 尝试预测
    res = svm_model_all_train_load.predict(deal_input)

    score = svm_model_all_train_load.decision_function(deal_input)
